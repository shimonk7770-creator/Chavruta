...
מריץ את כל הבדיקות תחת `server/test/` עם `node:test` המובנה ב-Node (לא דורש ספריית בדיקות נוספת) - 23 בדיקות בסה"כ.
מכסה ולידציית הרשמה (BR-001/002/003), לוגיקת הרשאות (AC-002/AC-003) ובניית לוח השנה (`server/utils/calendarGrid.js`).
